...
size_t detection_utils::vectorProduct(const std::vector<int64_t>& vector)
{
    if (vector.empty())
        return 0;
...
std::wstring detection_utils::charToWstring(const char* str)
{
...
std::vector<std::string> detection_utils::loadClassNames(const std::string& path)
{
    // load class names
    ...
    if (infile.good())
    {
        ...
        while (getline (infile, line))
        {
            if (line.back() == '\r')
                line.pop_back();
            classNamesList.emplace_back(line);
        }
        infile.close();
    }
    else
...
void detection_utils::visualizeDetection(cv::Mat& image, std::vector<Detection>& detections,
                               const std::vector<std::string>& classNamesList)
{
    for (const Detection& detection : detections)
    {
        ...
        std::string label = classNamesList[classId] + " 0." + std::to_string(conf);
...
void detection_utils::letterbox(const cv::Mat& image, cv::Mat& outImage,
...
template <typename T>
T detection_utils::clip(const T& n, const T& lower, const T& upper)
{
